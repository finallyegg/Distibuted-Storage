...
func main() {
	...
	addrPtr := flag.String("s", "", "servAddr")
	...
	case "get":
		...
		CmdGetFile(addr_1, args[0], args[1], ".")
	...
	case "anchor":
		CmdCreateAnchor(addr_1, args[0], *priKeyPtr)

	case "claim":
		CmdClaimAnchor(addr_1, args[0], args[1], args[2], *priKeyPtr)

	case "content":
		CmdContent(addr_1, args[0])

	case "rootanchor":
		fmt.Println(CmdRootanchor(addr_1, args[0]))
